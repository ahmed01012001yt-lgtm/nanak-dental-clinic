module.exports = async function handler(req, res) {
  ...
  // Force every browser to request the CURRENT hero redirect instead of a stale cached
  // redirect from the older image. The Supabase edge function currently points hero to
  // the dentist + patient clinic photo shown on desktop.
  html = html.replace(
    /https:\/\/oupovxujjshindzgflgz\.supabase\.co\/functions\/v1\/nanak-assets-v2\?img=hero(?:&[^\"']*)?/gi,
    'https://oupovxujjshindzgflgz.supabase.co/functions/v1/nanak-assets-v2?img=hero&asset=doctor-patient-v1'
  );

  // Keep appointment CTAs on the current page and open the modal.
  ...
  const marker = 'nanak-v5-same-hero-all-devices';
  const modal = `
...
<!-- ${marker} -->`;
  ...
  if (!html.includes(marker)) html = html.replace(/<\/body>/i, modal + '</body>');
  ...
  res.setHeader('x-nanak-version', 'v5-same-hero-all-devices');
